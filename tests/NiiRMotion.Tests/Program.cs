...
tests = [Sync("Static UI text has English localization coverage", StaticUiLocalizationCoverage), Sync("Games page buttons have English localization", GamesPageLocalizationCoverage), .. tests];
...
static void GamesPageLocalizationCoverage()
{
    var root = Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", ".."));
    var localization = File.ReadAllText(Path.Combine(root, "src", "NiiRMotion.App", "UiLocalization.cs"));
    foreach (var key in new[] { "⚙  AYARLAR", "▶  DOĞRULA VE OYUNU BAŞLAT", "GENEL HIZ", "AZAMİ HIZ", "KÜÇÜK HAREKET FİLTRESİ", "BAŞLAMA TEPKİSİ", "DURMA TEPKİSİ", "AYARLARI KAYDET" })
        Assert(localization.Contains($"[\"{key}\"]", StringComparison.Ordinal), $"Games page English translation is missing: {key}");
    var gamesPage = File.ReadAllText(Path.Combine(root, "src", "NiiRMotion.App", "MainWindow.xaml.cs"));
    Assert(gamesPage.Contains("UiLocalization.Apply(GamesPanel)", StringComparison.Ordinal), "Games page does not apply English localization to its panel.");
}
...
static void HeartbeatRefreshesPhonePresence() { var b = new byte[12]; System.Buffers.Binary.BinaryPrimitives.WriteInt32BigEndian(b, 0); System.Buffers.Binary.BinaryPrimitives.WriteInt64BigEndian(b.AsSpan(4), 1); Assert(OwoTrackPacketParser.TryParse(b, out var p) && p.Type == OwoTrackPacketType.Heartbeat, "Heartbeat parse failed."); PhonePresence.Mark("test-heartbeat:1234"); Assert(PhonePresence.TryGetFresh(out var ep) && ep == "test-heartbeat:1234", "Heartbeat must refresh phone presence."); }
static void PingPongRefreshesPhonePresence() { PhonePresence.Mark("test-pong:5678"); Assert(PhonePresence.TryGetFresh(out var ep) && ep == "test-pong:5678", "PingPong must keep phone presence fresh."); }
static void BroadcastHandshakePacketFormat() { var b = new byte[13]; b[0] = 3; System.Text.Encoding.ASCII.GetBytes("Hey OVR =D 5").CopyTo(b, 1); Assert(b[0] == 3, "Broadcast handshake type must be 3."); Assert(System.Text.Encoding.ASCII.GetString(b, 1, 12) == "Hey OVR =D 5", "Broadcast handshake string mismatch."); }
...
static void OptionalSensorFreshnessToleratesSmallClockSkew() { var f = new SensorFusionEngine(optionalFreshness: TimeSpan.FromMilliseconds(350)); var baseTicks = System.Diagnostics.Stopwatch.GetTimestamp(); var sampleTicks = baseTicks + System.Diagnostics.Stopwatch.Frequency / 100; f.ObservePhoneRhythm(0.9, sampleTicks); var snapshot = f.Update(baseTicks); Assert(snapshot.PhoneFresh, "Phone sample slightly in the future must still be considered fresh within the tolerance window."); var staleSnapshot = f.Update(baseTicks + (long)(System.Diagnostics.Stopwatch.Frequency * 0.6)); Assert(!staleSnapshot.PhoneFresh, "Phone sample beyond the freshness window must be stale."); }
