...
{
    if (!OperatingSystem.IsWindows()) return;
    using var map = System.IO.MemoryMappedFiles.MemoryMappedFile.CreateOrOpen(SharedMemoryHmdPoseSource.MappingName, 64);
    ...
    await using var source = new SharedMemoryHmdPoseSource(); await source.StartAsync();
