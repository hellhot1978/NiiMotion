...
public sealed class VrPanelWindow : Window
{
    ...
    public VrPanelWindow()
    {
        ...
        var start = new Button { Content = "▶  HAREKETİ BAŞLAT", Padding = new Thickness(14, 12, 14, 12), Background = MainWindow.Brush("#123D55"), BorderBrush = MainWindow.Brush("#2CB7F2") }; start.Click += (_, _) => _commands.Send(VrPanelCommand.StartLocomotion); Grid.SetColumn(start, 2); footer.Children.Add(start); Grid.SetRow(footer, 1); root.Children.Add(footer); Content = root;
