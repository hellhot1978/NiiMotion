...
public partial class GuidedCalibrationCaptureWindow : Window
{
    ...
    {
        var steps = Schedule(_sensor, _phase, _duration.TotalSeconds); var seconds = Math.Min(_duration.TotalSeconds, _elapsed.TotalSeconds); var step = steps.FirstOrDefault(x => seconds >= x.Start && seconds < x.End) ?? steps[^1]; var remaining = TimeSpan.FromSeconds(Math.Max(0, step.End - seconds));
    ...
    private static Step[] Schedule(SensorFamily sensor, int phase, double totalSeconds)
    {
        ...
        return raw.Select(s => new Step(s.Start * scale, s.End * scale, s.Title, s.Detail, s.Pose)).ToArray();
    }
