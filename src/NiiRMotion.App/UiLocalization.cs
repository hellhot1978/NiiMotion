...
public static class UiLocalization
{
    ...
    private static readonly IReadOnlyDictionary<string, string> English = new Dictionary<string, string>(StringComparer.Ordinal)
    {
        ...
        ["KALİBRASYON MERKEZİ"]="CALIBRATION CENTER", ["Önce cihazlarını hazırla"]="Prepare your devices first", ["Her cihaz bağlantıdan sonra üç adet kısa fazı tamamlar. Bu kayıtlar cihazı kullanıma açar."]="After connecting each device, complete three short base phases. These recordings prepare the device for use.",
        ["İki uyluk sensörü"]="Two thigh sensors", ["İki baldır sensörü"]="Two calf sensors", ["TEMEL FAZ  0/3"]="BASE PHASES  0/3", ["✓ KULLANIMA HAZIR"]="✓ READY TO USE", ["İSTEĞE BAĞLI · EK KAYIT"]="OPTIONAL · EXTRA RECORDING", ["Temel fazlardan sonra tek cihaz veya istediğin kombinasyonla ek kayıt yap."]="After the base phases, add an extra recording with one device or any available combination.",
        ["Henüz hareket cihazı seçmedin. Sol menüden Cihazlarım'ı aç."]="No motion device is selected yet. Open My Devices from the left menu.", ["SteamVR başlık yönü ve dönüş örneklerini oyun hareketi üretmeden doğrular"]="Validates SteamVR headset direction and turn samples without generating game movement", ["Bu kombinasyonla yeni kayıt ekle"]="Add a new recording with this combination", ["Önce bu cihazların temel kalibrasyonlarını tamamla"]="Complete base calibration for these devices first",
        ...
        ["TEMEL KALİBRASYON"]="BASE CALIBRATION", ["3 faz · her biri 5 dakika"]="3 phases · 5 minutes total", ["Tamamlanan bir faz hatalıysa yanındaki yeniden çek düğmesini kullanabilirsin."]="If a completed phase is incorrect, use its retake button.",
        ...
        ["Sol bacak"]="Left leg", ["Sağ bacak"]="Right leg", ["Telefon"]="Phone", ["bağlı"]="connected", ["Alyx telemetri bağlantısı kapandı."]="The Alyx telemetry connection closed.", ["Alyx oyun mesafesi bekleniyor…"]="Waiting for Alyx in-game distance…", ["ADIM"]="STEPS", ["SÜRE"]="DURATION", ["Yönerge burada görünecek"]="Instructions will appear here",
        ...
        ["Birlikte çalışma kalibrasyonu"]="Combined operation calibration", ["Aktif profil"]="Active profile", ["3 FAZ · 15 DAKİKA"]="3 PHASES · 15 MINUTES", ["3 FAZ · 6 DAKİKA"]="3 PHASES · 6 MINUTES", ["00:00 / 02:00"]="00:00 / 02:00", ["BU KAYIT NE YAPAR?"]="WHAT DOES THIS RECORDING DO?", ["Sensörleri aynı zaman çizgisinde ölçer"]="Measures sensors on the same timeline", ["Faz 1 ile başla."]="Start with Phase 1.",
        ...
        ["HMD yön doğrulaması"]="HMD direction validation", ["Başlık yön ve dönüş doğrulaması"]="Headset direction and turn validation", ["İSTEĞE BAĞLI · HMD"]="OPTIONAL · HMD", ["DOĞRULAMAYI AÇ  →"]="OPEN VALIDATION  →",
        ...
        ,["NiiMotion · Başlangıç Rehberi"]="NiiMotion · Getting Started", ["TERCİHLER"]="PREFERENCES", ["GÜNCELLEMELERİ KONTROL ET"]="CHECK FOR UPDATES", ["UYGULAMA GÜNCELLEMESİ"]="APP UPDATE", ["Yeni NiiMotion sürümünü denetle"]="Check for a new NiiMotion version", ["KONTROL ET"]="CHECK", ["⚙  AYARLAR"]="⚙  SETTINGS", ["▶  DOĞRULA VE OYUNU BAŞLAT"]="▶  VERIFY & START GAME", ["ARAYÜZ DİLİ"]="INTERFACE LANGUAGE", ["Uygulamanın görüntüleneceği dili seç"]="Choose the language used by the app"
        ,["OYUN İÇİ HIZ UYUMU"]="IN-GAME PACE TUNING", ["DAHA HIZLI OLMALI"]="SHOULD BE FASTER", ["HIZ DOĞRU"]="PACE IS RIGHT", ["DAHA YAVAŞ OLMALI"]="SHOULD BE SLOWER", ["SON HIZ AYARINI GERİ AL"]="UNDO LAST PACE CHANGE", ["ÖĞRENİLEN HIZI SIFIRLA"]="RESET LEARNED PACE", ["TÜM AYARLARI SIFIRLA"]="RESET ALL SETTINGS", ["GENEL HIZ"]="OVERALL SPEED", ["AZAMİ HIZ"]="MAXIMUM SPEED", ["KÜÇÜK HAREKET FİLTRESİ"]="SMALL MOTION FILTER", ["BAŞLAMA TEPKİSİ"]="START RESPONSE", ["DURMA TEPKİSİ"]="STOP RESPONSE", ["AYARLARI KAYDET"]="SAVE SETTINGS", ["Otomatik oyun ölçümü açık. Sonuç sana yavaş veya hızlı gelirse aşağıdan elle düzeltebilirsin."]="Automatic in-game measurement is active. If the result feels slow or fast, you can adjust it manually below.", ["Öğrenilmiş oyun hızı: {VALUE0}×"]="Learned game pace: {VALUE0}×", ["3 faz · {VALUE0} + {VALUE1} + {VALUE2} ({VALUE3}dk)"]="3 phases · {VALUE0} + {VALUE1} + {VALUE2} ({VALUE3}min)", ["PS Move pil seviyesi düşük: {VALUE0} ({VALUE1}%)"]="PS Move battery low: {VALUE0} ({VALUE1}%)"
        ,["BİRLİKTE ÇALIŞMA KALİBRASYONU GEREKİYOR"]="COMBINED CALIBRATION REQUIRED", ["Birlikte çalışma kalibrasyonu gerekli"]="Combined calibration required", ["Seçili cihazların birlikte çalışma kalibrasyonunu tamamla."]="Complete combined calibration for the selected devices.", ["Seçili cihaz kombinasyonunun üç ortak fazını tamamla. SteamVR başlatılmadı."]="Complete the three combined phases for the selected device combination. SteamVR was not started.", ["Birlikte çalışma kalibrasyonunu tamamla"]="Complete combined calibration"
        ,["Seçili cihaz kombinasyonunun birlikte çalışma kalibrasyonu tamamlanmadı."]="Combined calibration for the selected device combination is incomplete.", ["Telefon ve Balance Board birlikte çalışma kalibrasyonu tamamlanmadı."]="Combined calibration for Phone and Balance Board is incomplete."
        ,["BİRLİKTE ÇALIŞMA MODELLERİ"]="COMBINED OPERATION MODELS", ["Cihaz kombinasyonunu kalibre et"]="Calibrate a device combination", ["Her çoklu profil kendi üç ortak fazını ve yerel uyum modelini kullanır."]="Each multi-device profile uses its own three combined phases and local agreement model.", ["ORTAK KALİBRASYONU AÇ"]="OPEN COMBINED CALIBRATION", ["✓ 3/3 · ORTAK MODEL HAZIR"]="✓ 3/3 · COMBINED MODEL READY", ["ORTAK FAZ  {VALUE0}/3"]="COMBINED PHASE  {VALUE0}/3", ["Bu kombinasyonun ortak fazlarını aç"]="Open the combined phases for this combination"
        ,["▶ FAZ {VALUE0}'Ü BAŞLAT · 2 DK"]="▶ START PHASE {VALUE0} · 2 MIN", ["○ FAZ {VALUE0} · 2 DK"]="○ PHASE {VALUE0} · 2 MIN"
        ,["Fazı sil ve yeniden çek"]="Delete and retake phase", ["Ortak fazı yeniden çek"]="Retake combined phase", ["Faz {VALUE0} ve sonraki ortak fazlar silinip yeniden kayda açılsın mı?"]="Delete phase {VALUE0} and later combined phases and reopen them for recording?"
        ...
        ["NiiMotion kapalı · özgün VR kontrolü"]="NiiMotion off · native VR controls", ["En güçlü bacak doğrulaması"]="Strongest leg validation", ["Doğal yerinde yürüyüş"]="Natural walk-in-place", ["Basınç tabanlı deneysel hareket"]="Experimental pressure-based movement", ["Telefon tabanlı deneysel hareket"]="Experimental phone-based movement", ["DENEYSEL"]="EXPERIMENTAL", ["SANA UYGUN PROFİLLER"]="RECOMMENDED PROFILES", ["VR El Kontrolü"]="VR Hand Control",
        ["1. Cihazlarını seç"]="1. Select your devices", ["Sahip olduğun sensörleri Cihazlarım bölümünde işaretle."]="Select the sensors you own under My Devices.", ["2. Temel kalibrasyon"]="2. Base calibration", ["Seçtiğin her hareket cihazının üç temel fazını tamamla."]="Complete the three base phases for every selected motion device.", ["Tamamlanan fazlardan yerel hareket modeli yeniden oluşturulmalı."]="The local motion model must be rebuilt from the completed phases.", ["3. Yürüyüş profilini seç"]="3. Select a walking profile", ["Genel Bakış ekranında hazır cihazlarına uygun profili seç."]="Select a profile matching your ready devices on the Overview page.", ["4. Oyunu doğrula"]="4. Validate a game", ["Bir VR oyunu yerel başlatma zinciriyle başarıyla doğrulandı."]="A VR game was successfully validated through the local launch sequence.", ["Oyunlar bölümünden VR oyununu seç; NiiMotion bağlantıları sırayla kontrol etsin."]="Select a VR game under Games and let NiiMotion check each connection in order.",
        ...
        ["KART SIFIRLANIYOR"]="ZEROING BOARD", ["KART BOŞ KALSIN"]="KEEP THE BOARD EMPTY", ["ŞİMDİ KARTA ÇIK"]="STEP ONTO THE BOARD NOW", ["KART BEKLENİYOR"]="WAITING FOR BOARD", ["TAM KALİBRASYONA HAZIRLAN"]="PREPARE FOR FULL CALIBRATION", ["DOĞAL YÜRÜYÜŞE HAZIRLAN"]="PREPARE FOR NATURAL WALKING", ["TAM KALİBRASYON"]="FULL CALIBRATION", ["ÖLÇÜLÜYOR"]="MEASURING", ["DOĞAL YERİNDE YÜRÜ"]="WALK NATURALLY IN PLACE", ["ŞİMDİ SABİT DUR"]="STAND STILL NOW", ["ŞİMDİ KARTTAN İN"]="STEP OFF THE BOARD NOW", ["TAMAMLANDI"]="COMPLETE", ["TAMAM"]="OK", ["↻  KONTROL ET"]="↻  CHECK", ["Quest ve Virtual Desktop oturumu bekleniyor…"]="Waiting for the Quest and Virtual Desktop session…", ["USB TAMAM"]="USB COMPLETE", ["KAYDEDİLDİ"]="SAVED", ["ÖLÇÜM ALINAMADI"]="MEASUREMENT FAILED",
        ...
        ["Android telefon"]="Android phone", ["Joy-Con çifti · temel kalibrasyon"]="Joy-Con pair · base calibration", ["PS Move çifti · temel kalibrasyon"]="PS Move pair · base calibration", ["Android telefon · temel kalibrasyon"]="Android phone · base calibration", ["Wii Balance Board · temel kalibrasyon"]="Wii Balance Board · base calibration", ["Joy-Con çifti"]="Joy-Con pair", ["PS Move çifti"]="PS Move pair",
        ["1. İki Joy-Con'u Windows Bluetooth'a bağla.\n2. Solu sol, sağı sağ uyluğa; kalça ile diz arasına sabitle.\n3. Düğmeler dışa, analog çubuklar yukarı baksın."]="1. Pair both Joy-Cons through Windows Bluetooth.\n2. Secure left to the left thigh and right to the right thigh, between hip and knee.\n3. Buttons face outward and analog sticks point up.",
        ["1. Sol Move'u tek başına USB ile bağla; NiiMotion eşleştirip kırmızı atar.\n2. Aynı işlemi sağ Move için yap; otomatik mavi atanır.\n3. Kabloları çıkarıp Move düğmelerine bas, sonra küre yukarı bakacak şekilde diz altına/baldıra sabitle."]="1. Connect only the left Move by USB; NiiMotion pairs it and assigns red.\n2. Repeat for the right Move; blue is assigned automatically.\n3. Unplug the cables, press each Move button, then secure below the knee on the calf with the sphere pointing up.",
        ["1. Android telefona owoTrack kur ve aç.\n2. Telefon ile bilgisayarı aynı ağa bağla.\n3. Göğüste yatay; ekran sana, üst kenar sola bakacak şekilde sabitle."]="1. Install and open owoTrack on the Android phone.\n2. Connect the phone and PC to the same network.\n3. Secure it horizontally on your chest, with the screen facing you and the top edge pointing left.",
        ["1. Windows Bluetooth'tan Nintendo RVL-WBC-01 olarak eşleştir.\n2. Gerekirse karttaki SYNC ve güç düğmesini kullan.\n3. İlk kontrol sırasında kart boş ve düz zeminde olsun."]="1. Pair it as Nintendo RVL-WBC-01 in Windows Bluetooth.\n2. Use the SYNC and power buttons on the board if needed.\n3. Keep the board empty on a level surface during the first check.",
        ["İki Joy-Con'u Windows Bluetooth'a bağla."]="Connect both Joy-Cons through Windows Bluetooth.", ["İki atanmış PS Move'u Bluetooth ile bağla."]="Connect both assigned PS Move controllers through Bluetooth.", ["Telefonda owoTrack'i başlat ve aynı ağa bağlan."]="Start owoTrack on the phone and connect it to the same network.", ["Balance Board'u Windows Bluetooth'a bağla ve kartı boş bırak."]="Connect the Balance Board through Windows Bluetooth and keep it empty.",
        ["Sabit ve rahat dur"]="Stand still and relaxed", ["Ayakların yerde; dizlerin kilitli olmasın."]="Keep both feet down without locking your knees.", ["Yavaşça yerinde yürü"]="Walk slowly in place", ["Olduğun yerden ayrılmadan küçük, düzenli adımlar at."]="Take small, regular steps without leaving your position.", ["Dur ve sabit kal"]="Stop and remain still", ["Ayaklarını yere koy; tamamen hareketsiz kal."]="Put both feet down and remain completely still.", ["Yavaş yürüyüşe devam et"]="Continue walking slowly", ["Aynı doğal küçük adımları sürdür."]="Continue the same natural small steps.", ["Dur ve rahatla"]="Stop and relax", ["Faz bitene kadar sabit kal."]="Remain still until the phase ends.", ["Faz tamamlanacak"]="Phase will complete", ["Sensör verisi güvenli kayıt klasörüne yazılıyor."]="Sensor data is being written to the protected recording folder.",
        ["★ 2 dakikalık tam kalibrasyon"]="★ 2-minute full calibration", ["Ortada sabit duruş"]="Centered standing", ["Yavaş yerinde yürüyüş"]="Slow walk in place", ["Doğal yerinde yürüyüş"]="Natural walk in place", ["Hızlı yerinde yürüyüş"]="Fast walk in place", ["Sağa-sola dönüş"]="Turn right and left", ["Yürü → dur → karttan in"]="Walk → stop → step off",
        ["MODEL SAĞLIĞI"]="MODEL HEALTH", ["NiiMotion · Ortak Model Sağlığı"]="NiiMotion · Combined Model Health", ["Ortak model sağlığı"]="Combined model health", ["Her cihaz kombinasyonunun kayıt, model ve geçmiş durumunu gösterir."]="Shows recording, model and history status for every device combination.",
        ["✓ Kayıtlar tamamlandı · ortak model yerel olarak hazırlanıyor."]="✓ Recordings complete · preparing the combined model locally.", ["Kayıt duraklatıldı; bu süre modele eklenmeyecek."]="Recording paused; paused time will not be added to the model.", ["Faz {VALUE0} yeniden kayda açıldı."]="Phase {VALUE0} was reopened for recording.",
        ["✓ Önceki ortak model geri yüklendi. Tekil cihaz modelleri değiştirilmedi."]="✓ The previous combined model was restored. Individual device models were not changed.", ["✓ Ortak model hazır · kalite %{VALUE0} · {VALUE1} örnek · ritim toleransı {VALUE2} Hz · kesinti payı {VALUE3} ms"]="✓ Combined model ready · quality {VALUE0}% · {VALUE1} samples · cadence tolerance {VALUE2} Hz · dropout allowance {VALUE3} ms",
        ["Ortak faz {VALUE0}/3"]="Combined phase {VALUE0}/3", ["Kalite %{VALUE0} · {VALUE1} örnek"]="Quality {VALUE0}% · {VALUE1} samples", ["MODEL EKSİK"]="MODEL MISSING", ["KALİBRASYON GEREKİYOR"]="CALIBRATION REQUIRED", ["ÖNCEKİ ORTAK MODELE DÖN"]="RESTORE PREVIOUS COMBINED MODEL"
    };
    private static readonly IReadOnlyList<TemplateTranslation> EnglishTemplates = English
        .Where(x => x.Key.Contains("{VALUE", StringComparison.Ordinal))
        .Select(x => TemplateTranslation.Create(x.Key, x.Value)).ToArray();
    ...
    [
        ...
        ("Otomatik oyun ölçümü açık", "Automatic in-game measurement is active"),
        ("Öğrenilmiş oyun hızı", "Learned game pace"),
    ...
    private static string Translate(string value)
    {
        ...
        foreach (var template in EnglishTemplates)
        ...
        result = Regex.Replace(result, @"^TEMEL FAZ\s+(\d+)/3$", "BASE PHASES  $1/3", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
        result = Regex.Replace(result, @"Son kayıt hazır\s*·", "Latest recording ready ·", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
        result = Regex.Replace(result, @"%\s*takip\b", "% tracked", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
