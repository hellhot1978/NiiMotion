...
public partial class RecoveryCenterWindow : Window
{
    ...
    private void RestoreClick(object sender, RoutedEventArgs e)
    {
        ...
        if (UiLocalization.ShowMessage(this, "Seçili yedek geri yüklensin mi? Mevcut durum önce ayrıca saklanacak.", "Geri yükle", MessageBoxButton.YesNo, MessageBoxImage.Warning) != MessageBoxResult.Yes) return;
    ...
    {
        var first = UiLocalization.ShowMessage(this, "Kişisel hareket modelleri, temel kalibrasyon ilerlemesi ve ham eğitim kayıtları sıfırlanacak. PS Move/Joy-Con eşleştirmeleri, cihaz tercihleri ve oyun ayarları korunacak. Devam edilsin mi?", "Öğrenilmiş veriyi sıfırla", MessageBoxButton.YesNo, MessageBoxImage.Warning);
        if (first != MessageBoxResult.Yes) return;
        var second = UiLocalization.ShowMessage(this, "İşlemden önce tam geri dönüş ZIP'i oluşturulacak. Sıfırlamayı onaylıyor musun?", "Son onay", MessageBoxButton.YesNo, MessageBoxImage.Warning);
        if (second != MessageBoxResult.Yes) return;
        var result = new LearnedMotionDataService().Reset();
        UiLocalization.ShowMessage(this, $"Öğrenilmiş hareket verileri sıfırlandı.\n\nGeri dönüş arşivi:\n{result.BackupPath}", "Sıfırlama tamamlandı", MessageBoxButton.OK, MessageBoxImage.Information);
