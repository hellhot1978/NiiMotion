...
public sealed class GettingStartedWindow : Window
{
    public GettingStartedWindow(UserHardwareInventory inventory, CalibrationProgressDocument progress, bool settingsOnly = false)
    {
        ...
        var save = new Button { Content = "KAYDET VE DEVAM ET", Padding = new Thickness(20, 10, 20, 10), Background = MainWindow.Brush("#087DC4") }; save.Click += (_, _) => { store.Save(prefs with { TextScale = scale.Value, HighContrast = contrast.IsChecked == true, ReducedMotion = reduced.IsChecked == true, OnboardingComplete = true }); DialogResult = true; Close(); };
