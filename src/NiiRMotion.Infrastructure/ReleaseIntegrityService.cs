...
public static class ReleaseIntegrityService
{
    ...
    {
        var names = new[] { "NiiRMotion.App.exe", Path.Combine("OpenXRLayer", "niirmotion_openxr.json"), Path.Combine("OpenXRLayer", "bin", "win64", "niirmotion_openxr.dll"), Path.Combine("OpenVRDriver", "bin", "win64", "driver_niirmotion.dll"), Path.Combine("VrOverlay", "NiiMotion.VrOverlay.exe"), Path.Combine("VrOverlay", "openvr_api.dll"), Path.Combine("VrOverlay", "niirmotion.vrmanifest"), Path.Combine("VrOverlay", "dashboard-icon.png") };
