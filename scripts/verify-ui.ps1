...
foreach ($case in $cases) {
    ...
    try {
        ...
        if (-not $process.WaitForExit(15000)) { $process.Kill(); throw "UI smoke process did not exit: $($case.Name)" }
...
foreach ($language in @('tr', 'en')) {
    ...
    if (-not $process.WaitForExit(15000)) { $process.Kill(); throw "Getting Started UI process did not exit: $language" }
...
foreach ($language in @('tr', 'en')) {
    ...
    while ((-not (Test-Path -LiteralPath $path) -or (Get-Item -LiteralPath $path -ErrorAction SilentlyContinue).Length -lt 10000) -and [DateTime]::UtcNow -lt $deadline) { Start-Sleep -Milliseconds 100 }
    if (-not $process.HasExited -and -not $process.WaitForExit(15000)) { $process.Kill(); throw "Calibration center process did not exit: $language" }
...
foreach ($dialog in $dialogCases) {
    foreach ($language in @('tr', 'en')) {
        ...
        if (-not $process.HasExited -and -not $process.WaitForExit(12000)) { $process.Kill(); throw "Dialog UI process did not exit: $($dialog.Name)-$language" }
